...
export default function CollaboratePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus('success');
      setFeedbackMessage('Thank you for your collaboration request! I will review it and get back to you soon.');
      // Clear form
      setName('');
      setEmail('');
      setPhone('');
      ...
      setStatus('error');
      setFeedbackMessage('There was an error submitting your request. Please try again later.');
    }
  };

  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-4xl font-bold text-center mb-8">Collaborate With Me</h1>
      <p className="text-center text-gray-600 mb-8">Have an idea for a project or want to work together? Let's connect.</p>
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
        ...
        <div>
          <label htmlFor="idea" className="block text-sm font-medium text-gray-700">Collaboration Idea</label>
          <textarea id="idea" value={idea} onChange={(e) => setIdea(e.target.value)} required rows={5} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Briefly describe your project or idea..."></textarea>
        </div>
        <div>
